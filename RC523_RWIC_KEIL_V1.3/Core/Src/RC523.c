...
uint8_t RC523ChannlxReadData(uint8_t* AllData)
{
	...
	for(i = 0; i < 3; i++)
	{
		...
		if(status == MI_OK)
		{
				status = PcdRead((i*4) + 1, AES_TempData); // 0  32  64    1  5  9
				invcipher(AES_TempData,AES_KEY);
				memcpy(AllData + 16 * i * 2, AES_TempData, 16);
		}
		else
		{
				return status;
		...
		if(status == MI_OK)
		{
				...
				status = PcdRead((i*4) + 2, AES_TempData); // 16 48  80 2 6 10
				invcipher(AES_TempData,AES_KEY);
				memcpy(AllData + 16 * (2 * i + 1), AES_TempData, 16);
		}
		else
		{
				return status;
...
uint8_t RC523ChannlxWriteData(uint8_t* AllData)
{
	...
	for(i = 0; i < 3; i++)
	{
		...
		if(status == MI_OK)
		{
				memcpy(AES_TempData,AllData + 16 * i * 2, 16);
				cipher(AES_TempData,AES_KEY);
				status = PcdWrite((i*4) + 1, AES_TempData); // 0  32  64    1  5  9
		}
		else
		{
				return status;
		...
		if(status == MI_OK)
		{
				memcpy(AES_TempData,AllData + 16 * (2 * i + 1), 16);
				cipher(AES_TempData,AES_KEY);
				status = PcdWrite((i*4) + 2, AES_TempData); // 16 48  80 2 6 10
		}
		else
		{
				return status;
